...
func (sr *SecretsRepo) Create(ctx context.Context, secret domain.Secret) error {
	...
	if err != nil {
		...
		sr.logger.ErrorContext(ctx, "failed to create secret", logger.ErrAttr(err))
...
func (sr *SecretsRepo) GetUserSecrets(ctx context.Context, userID string) (*[]domain.Secret, error) {
	...
	if err != nil {
		sr.logger.ErrorContext(ctx, "failed to get user secrets", logger.ErrAttr(err))
	...
	for rows.Next() {
		...
		if err != nil {
			sr.logger.ErrorContext(ctx, "failed to scan secret", logger.ErrAttr(err))
			return nil, fmt.Errorf("%s: %w", op, err)
		}
	...
	if rows.Err() != nil {
		sr.logger.ErrorContext(ctx, "failed to iterate over secrets", logger.ErrAttr(err))
		return nil, fmt.Errorf("%s: %w", op, err)
	}
...
func (sr *SecretsRepo) GetSecretByID(ctx context.Context, secretID string) (*domain.Secret, error) {
	...
	if err != nil {
		...
		sr.logger.ErrorContext(ctx, "failed to get secret by id", logger.ErrAttr(err))
		return nil, fmt.Errorf("%s: %w", op, err)
	}
