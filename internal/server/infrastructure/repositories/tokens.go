...
func (tr *TokenRepo) Create(ctx context.Context, token RefreshToken) error {
	...
	if err != nil {
		tr.logger.ErrorContext(ctx, "failed to store refresh token", slog.String("user_id", token.UserID), slog.String("token", token.Token), logger.ErrAttr(err))
...
func (tr *TokenRepo) GetRefreshToken(ctx context.Context, tokenID string) (*RefreshToken, error) {
	...
	if err != nil {
		tr.logger.ErrorContext(ctx, "failed to retrieve refresh token", slog.String("token_id", tokenID), logger.ErrAttr(err))
