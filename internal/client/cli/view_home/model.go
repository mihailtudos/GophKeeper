...
const (
	CreateNewSecretMessageKey = "create_new_secret"
)
	...
	case tea.KeyMsg:
		...
		case "ctrl+n":
			return m, func() tea.Msg {
				return messages.ActionMsg{Value: CreateNewSecretMessageKey}
			}
		}
	}
	return m, nil
...
func (m Model) View() string {
	s := strings.Builder{}
	fmt.Fprintf(&s, "%s\n\n%s\n\n", headerStyle.Render(m.AppName), "Your secrets:")
	...
	//fmt.Fprintf(&s, "\n\n%s\t%s\n\n", buttons[0], buttons[1])
	s.WriteString("\nctrl+n - new secret | ctrl+l - logout | ctrl+c, esc - quit\n\n")
