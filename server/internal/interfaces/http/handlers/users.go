...
func (h *Handler) GetSecrets(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.Unmarshal(data, &password); err != nil {
		h.Logger.Error("failed to decode password", pkg.ErrAttr(err))
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		h.Logger.Error("failed to get user secrets", pkg.ErrAttr(err))
	...
	if err != nil {
		h.Logger.Error("failed to marshal secrets", pkg.ErrAttr(err))
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
